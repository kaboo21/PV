...
namespace Chapter_6._6
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //Students with grades "5" move to top
            for (int i = 0, indexGoodStuden=0; i < students.Length; i++)
            {
                if (students[i].math ==5 && students[i].computerScience == 5 && students[i].philosophy == 5 &&
                    students[i].physicalEducation == 5)
                {
                    Student temp = students[indexGoodStuden];
                    students[indexGoodStuden] = students[i];
                    students[i] = temp;
                    indexGoodStuden++;
                }
            }
